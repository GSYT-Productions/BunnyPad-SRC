...
"""
try:
    import sys, os, time, platform, distro, unicodedata, textwrap, datetime, re, random, webbrowser, psutil, shutil, \
        subprocess
    from PyQt6.QtCore import *
    from fpdf import FPDF
    from PyQt6.QtWidgets import (QApplication, QMainWindow, QTextEdit, QFileDialog, QWidget, QDialog, QMenuBar, QMenu,
                                 QToolBar, QStatusBar, QVBoxLayout, QHBoxLayout, QDockWidget, QLabel, QToolTip,
...
class AboutDialog(QDialog):
    def __init__(self, *args, **kwargs):
        ...
        layout.addWidget(
            QLabel("A Notepad Clone named in part after Innersloth's Off-Topic Regular, PBbunnypower [aka Bunny]"))
        layout.addWidget(
            QLabel("Copyright © 2023-2024 GSYT Productions, LLC\nCopyright © 2024 The BunnyPad Contributors"))
...
class DownloadOptions(QDialog):
    def __init__(self, parent=None):
        ...
        for full_name, object_name in button_names.items():
            ...
            button.clicked.connect(getattr(self,
                                           f"on_{object_name.replace(' ', '_').replace('-', '_').replace('(', '').replace(')', '')}_clicked"))
...
class Notepad(QMainWindow):
    def __init__(self):
        ...
        self.toggle_character_map_toolbar_action = QAction(QIcon("images/charmap.png"), "Character Map", self,
                                                           checkable=True)
    ...
    def warn_unsaved_changes(self):
        ret = QMessageBox.warning(self, "BunnyPad",
                                  "The document has been modified. Would you like to save your changes?",
